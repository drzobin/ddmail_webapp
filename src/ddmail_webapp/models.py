...
class Alias(db.Model):
    ...
    src_account_domain_id = db.mapped_column(
        db.Integer, ForeignKey("account_domains.id"), nullable=True
    )
    src_global_domain_id = db.mapped_column(
        db.Integer, ForeignKey("global_domains.id"), nullable=True
    )
...
class Email(db.Model):
    ...
    account_domain_id = db.mapped_column(
        db.Integer, ForeignKey("account_domains.id"), nullable=True
    )
    global_domain_id = db.mapped_column(
        db.Integer, ForeignKey("global_domains.id"), nullable=True
    )
    openpgp_public_key_id = db.mapped_column(
        db.Integer, ForeignKey("openpgp_public_keys.id"), nullable=True
    )
...
class Authenticated(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True, nullable=False)
    cookie = db.Column(db.String(128), unique=True, nullable=False)
