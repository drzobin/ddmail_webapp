...
def settings():
    """
    Display the main settings dashboard for authenticated users.

    This function serves as the central settings hub where users can access
    various account management options including password changes, email
    management, domain configuration, and account information.

    Returns:
        Response: Flask response with settings dashboard or login redirect

    Request Form Parameters:
        None: This endpoint does not require form parameters

    Error Responses:
        None: Unauthenticated users are redirected to login page

    Success Response:
        Renders settings.html template with user dashboard and account information
    """
...
def usage_and_funds():
    """
    Display account usage statistics and fund balance information.

    This function shows users their current account usage metrics including
    storage consumption, email counts, and available funds balance for
    account management and billing purposes.

    Returns:
        Response: Flask response with usage statistics or login redirect
    ...
    Success Response:
        Renders settings_usage_and_funds.html template with account usage and fund information
    """
    # Check if cookie secret is set.
    if not "secret" in session:
        current_app.logger.warning("secret is not in session")
...
def payment():
    """
    Display payment information and billing options.

    This function shows users payment methods, billing history, and options
    for account funding including bank transfer information and payment
    processing details for account activation.

    Returns:
        Response: Flask response with payment information or login redirect
    ...
    Success Response:
        Renders settings_payment.html template with payment and billing information
    """
    # Check if cookie secret is set.
    if not "secret" in session:
        current_app.logger.warning("secret is not in session")
    ...
    return render_template(
        ...
        payment_bankgiro=payment_bankgiro,
...
def payment_token():
    """
    Display the unique payment token for account identification.

    This function shows users their unique payment token which is used
    to identify payments and associate them with the correct account
    for billing and account activation purposes.

    Returns:
        Response: Flask response with payment token or login redirect
    ...
    Success Response:
        Renders settings_payment_token.html template with account payment token
    """
    # Check if cookie secret is set.
    if not "secret" in session:
        current_app.logger.warning("secret is not in session")
...
def settings_change_password_on_user():
    """
    Handle user password changes for the authenticated user.
    ...
    if current_user.account.is_enabled != True:
        ...
        return render_template(
            "message.html",
            headline="Change user password error",
            message="Failed to change users password beacuse this account is disabled. In order to enable the account you need to pay, see payments option in menu.",
            current_user=current_user,
        )
...
def settings_change_key_on_user():
    ...
    if current_user.account.is_enabled != True:
        ...
        return render_template(
            ...
            message="Failed to change users key beacuse this account is disabled. In order to enable the account you need to pay, see payments option in menu.",
            current_user=current_user,
        )
...
def settings_show_account_users():
    ...
    if current_user.account.is_enabled != True:
        ...
        return render_template(
            "message.html",
            headline="Show account users error",
            message="Failed to show account users beacuse this account is disabled. In order to enable the account you need to pay, see payments option in menu.",
            current_user=current_user,
        )
    ...
    return render_template(
        "settings_show_account_users.html", users=users, current_user=current_user
    )
...
def settings_remove_account_user():
    ...
    This function allows account administrators to remove users from their
    account, revoking their access to account resources and deleting their
    user records from the system.

    Returns:
        Response: Flask response with removal result or login redirect

    Request Form Parameters:
        remove_user (str): Username of the user to remove
        csrf_token (str): CSRF protection token for POST requests

    Error Responses:
        "Failed to remove account user beacuse this account is disabled": If account is not enabled
        "Failed to remove account user, you can not remove the same user as you are logged in as": If trying to remove self
        "Failed to removed account user, validation failed": If user validation fails
        "Failed to removed account user, illigal character in string": If username contains invalid characters
        "The CSRF token is missing": If CSRF token validation fails

    Success Response:
        GET: Renders remove_account_user.html template with user removal form
        POST: Renders template with success message confirming user removal
    """
    # Check if cookie secret is set.
    if not "secret" in session:
        current_app.logger.warning("secret is not in session")
    ...
    if current_user.account.is_enabled != True:
        ...
        return render_template(
            "message.html",
            headline="Remove account user error",
            message="Failed to remove account user beacuse this account is disabled. In order to enable the account you need to pay, see payments option in menu.",
            current_user=current_user,
        )
    ...
    if request.method == "GET":
        ...
        return render_template(
            "settings_remove_account_user.html", users=users, current_user=current_user
        )
    ...
    if request.method == "POST":
        ...
        if validators.is_username_allowed(remove_user_from_form) == False:
            current_app.logger.warning(
                ...
                + " can not be removed beacuse string validation failed"
            )
            return render_template(
                "message.html",
                headline="Remove user error",
                message="Failed to removed account user, illigal character in string.",
                current_user=current_user,
            )
        ...
        if is_user_mine != 1:
            current_app.logger.warning(
                "user "
                + current_user.user
                + " can not be removed beacuse user is not in db or is not owned by this account"
            )
            return render_template(
                "message.html",
                headline="Remove user error",
                message="Failed to removed account user, validation failed.",
                current_user=current_user,
            )
        ...
        if remove_user_from_form == current_user.user:
            current_app.logger.warning(
                "user "
                + current_user.user
                + " can not be removed beacuse user are logged in as this user"
            )
            return render_template(
                "message.html",
                headline="Remove user error",
                message="Failed to remove account user, you can not remove the same user as you are logged in as.",
                current_user=current_user,
            )
...
def settings_add_email():
    ...
    This function allows users to create additional email addresses under
    their account using either global domains or custom account domains.
    It validates the email format and creates the necessary database records.

    Returns:
        Response: Flask response with email creation result or login redirect

    Request Form Parameters:
        domain (str): Domain name for the email address
        email (str): Local part of the email address
        csrf_token (str): CSRF protection token for POST requests

    Error Responses:
        "Failed to add email beacuse this account is disabled": If account is not enabled
        "Failed to add email, email validation failed": If email format is invalid
        "Failed to add email, csrf validation failed": If CSRF validation fails
        "The CSRF token is missing": If CSRF token validation fails

    Success Response:
        GET: Renders add_email.html template with email creation form
        POST: Renders template with success message and new email details
    """
    # Check if cookie secret is set.
    if not "secret" in session:
        current_app.logger.warning("secret is not in session")
    ...
    if current_user.account.is_enabled != True:
        ...
        return render_template(
            "message.html",
            headline="Add email error",
            message="Failed to add email beacuse this account is disabled. In order to enable the account you need to pay, see payments option in menu.",
            current_user=current_user,
        )
    ...
    if request.method == "POST":
        if not form.validate_on_submit():
            current_app.logger.warning(
                ...
                + " account "
                + current_user.account.account
                + " failed to add email beacuse csrf validation failed"
            )
            return render_template(
                "message.html",
                headline="Add email error",
...
def settings_show_email():
    ...
    if current_user.account.is_enabled != True:
        ...
        return render_template(
            "message.html",
            headline="Show email error",
            message="Failed to show email beacuse this account is disabled. In order to enable the account you need to pay, see payments option in menu.",
            current_user=current_user,
        )
    ...
    return render_template(
        "settings_show_email.html", emails=emails, current_user=current_user
    )
...
def settings_remove_email():
    ...
    This function allows users to delete email addresses from their account.
    It communicates with external email removal services to properly clean
    up email data and remove database records.

    Returns:
        Response: Flask response with removal result or login redirect

    Request Form Parameters:
        remove_email (str): Email address to remove
        csrf_token (str): CSRF protection token for POST requests

    Error Responses:
        "Failed to remove email beacuse this account is disabled": If account is not enabled
        "Failed to removed email beacuse email remover service is unavalible": If external service fails
        "The CSRF token is missing": If CSRF token validation fails

    Success Response:
        GET: Renders remove_email.html template with email removal form
        POST: Renders template with success message confirming email removal
    """
    # Check if cookie secret is set.
    if not "secret" in session:
        current_app.logger.warning("secret is not in session")
    ...
    if request.method == "GET":
        ...
        return render_template(
            "settings_remove_email.html", emails=emails, current_user=current_user
        )
    if request.method == "POST":
        ...
        if validators.is_domain_allowed(domain) == False:
            current_app.logger.warning(
                ...
                + " failed to remove email "
                + remove_email_from_form
                + " beacuse domain validation failed"
            )
            return render_template(
                "message.html",
                headline="Remove email error",
        ...
        is_email_mine = (
            db.session.query(Email)
            .filter(
                Email.email == remove_email_from_form,
                Email.account_id == current_user.account_id,
            )
            .count()
        )
        if is_email_mine != 1:
            current_app.logger.warning(
                ...
                + " failed to remove email "
                + remove_email_from_form
                + " beacuse domain is not in db or is not owned by account"
            )
            return render_template(
                "message.html",
                headline="Remove email error",
        ...
        try:
            ...
            return render_template(
                "message.html",
                headline="Remove Email Error",
                message="Failed to removed email beacuse email remover service is unavalible.",
                current_user=current_user,
            )
        ...
        if r_respone.status_code != 200 or r_respone.content != b"done":
            current_app.logger.error(
                ...
                + " failed to remove email "
                + remove_email_from_form
                + " beacuse ddmail email remover service returned error"
            )
            return render_template(
                "message.html",
                headline="Remove email error",
        ...
        return render_template(
            "message.html",
            headline="Remove Email Account",
            message="Successfully removed email.",
            current_user=current_user,
        )
...
def settings_change_password_on_email():
    ...
    if current_user.account.is_enabled != True:
        ...
        return render_template(
            ...
            message="Failed to change password on email account beacuse this account is disabled. In order to enable the account you need to pay, see payments option in menu.",
            current_user=current_user,
        )
    ...
    if request.method == "GET":
        ...
        return render_template(
            "settings_change_password_on_email.html",
            form=form,
            emails=emails,
            current_user=current_user,
        )
    ...
    if request.method == "POST":
        ...
        change_password_on_email_from_form = request.form[
            "change_password_on_email"
        ].strip()
        ...
        if validators.is_email_allowed(change_password_on_email_from_form) == False:
            current_app.logger.warning(
                ...
                + " account "
                + current_user.account.account
                + " failed to change password on email "
            ...
            return render_template(
                "message.html",
                headline="Change password on email account error",
                message="Failed to change password on email account, validation failed.",
                current_user=current_user,
            )
        ...
        if (
            validators.is_password_allowed(current_cleartext_password_from_form)
            == False
        ):
            current_app.logger.warning(
                "user "
                ...
                + " failed to change password on email "
                + change_password_on_email_from_form
                + " beacuse current password validation failed"
            )
            return render_template(
                "message.html",
                headline="Change password on email account error",
                message="Failed to change password on email account, validation failed on current password.",
                current_user=current_user,
            )
        ...
        is_email_mine = (
            db.session.query(Email)
            .filter(
                Email.email == change_password_on_email_from_form,
                Email.account_id == current_user.account_id,
            )
            .count()
        )
        if is_email_mine != 1:
            current_app.logger.warning(
                ...
                + " failed to change password on email "
                + change_password_on_email_from_form
                + " beacuse emails is not in db or is not owned by current user"
            )
            return render_template(
                "message.html",
                headline="Change password on email account error",
        ...
        try:
            if (
                ph.verify(
                    email_from_db.password_hash, current_cleartext_password_from_form
                )
                != True
            ):
                current_app.logger.warning(
                    ...
                    + " failed to change password on email "
                    + change_password_on_email_from_form
                    + " beacuse current password is wrong"
                )
                return render_template(
                    "message.html",
                    headline="Change password on email account error",
        ...
        dmcp_keyhandler_url = (
            current_app.config["DMCP_KEYHANDLER_URL"] + "/change_password_on_key"
        )
        dmcp_keyhandler_password = current_app.config["DMCP_KEYHANDLER_PASSWORD"]
        try:
            r_respone = requests.post(
                dmcp_keyhandler_url,
                {
                    "email": change_password_on_email_from_form,
                    "current_key_password": base64.b64encode(
                        bytes(current_cleartext_password_from_form, "utf-8")
                    ),
                    "new_key_password": base64.b64encode(
                        bytes(cleartext_password, "utf-8")
                    ),
                    "password": dmcp_keyhandler_password,
                },
            ...
            return render_template(
                "message.html",
                headline="Change Password On Email Account Error",
                message="Failed to change password on email account becuse dmcp keyhandler is unavalible",
                current_user=current_user,
            )
        ...
        if r_respone.status_code != 200 or r_respone.content != b"done":
            current_app.logger.error(
                ...
                + " failed to change password on email "
                + change_password_on_email_from_form
                + " beacuse dmcp keyhandler service returned error"
            )
            return render_template(
                "message.html",
                headline="Change password on email account error",
                message="Failed to change password on email account, failed to change password on encryption key.",
                current_user=current_user,
            )
...
def settings_show_openpgp_public_keys():
    ...
    if current_user.account.is_enabled != True:
        ...
        return render_template(
            "message.html",
            headline="Show openpgp public keys error",
            message="Failed to show openpgp public keys beacuse this account is disabled. In order to enable the account you need to pay, see payments option in menu.",
            current_user=current_user,
        )

    keys = db.session.query(Openpgp_public_key).filter(
    ...
    current_app.logger.debug(
        ...
        + " account "
        + current_user.account.account
        + " show openpgp public keys"
    )
    return render_template(
        "settings_show_openpgp_public_keys.html", keys=keys, current_user=current_user
    )
...
def settings_upload_openpgp_public_key():
    """
    Upload a new OpenPGP public key to the user's account.

    This function allows users to upload OpenPGP public keys for email
    encryption. It validates the key format, communicates with external
    key handlers, and stores the key information in the database.

    Returns:
        Response: Flask response with upload result or login redirect

    Request Form Parameters:
        openpgp_public_key (str): The OpenPGP public key content
        csrf_token (str): CSRF protection token for POST requests

    Error Responses:
        "Failed to upload openpgp public key beacuse this account is disabled": If account is not enabled
        "Failed to upload openpgp public key, validation failed": If key validation fails
        "Failed to upload openpgp public key, openpgp keyhandler service is unavalible": If external service fails
        "The CSRF token is missing": If CSRF token validation fails

    Success Response:
        GET: Renders upload_openpgp_public_key.html template with key upload form
        POST: Renders template with success message confirming key upload
    """
    # Check if cookie secret is set.
    if not "secret" in session:
        current_app.logger.warning("secret is not in session")
    ...
    if current_user.account.is_enabled != True:
        ...
        return render_template(
            ...
            message="Failed to upload openpgp public key beacuse this account is disabled. In order to enable the account you need to pay, see payments option in menu.",
            current_user=current_user,
        )
    ...
    if request.method == "POST":
        ...
        try:
            r_respone = requests.post(
                openpgp_keyhandler_url,
                {
                    ...
                    "keyring": current_user.account.account,
            ...
            current_app.logger.error(
                ...
                + " account "
                + current_user.account.account
                + " faild to upload openpgp public key beacuse openpgp keyhandler service do not answer"
            )
            return render_template(
                "message.html",
                headline="Upload OpenPGP Public Key Error",
        ...
        if r_respone.status_code != 200 or "done fingerprint: " not in str(
            ...
            current_app.logger.error(
                ...
                + " account "
                + current_user.account.account
                + " faild to upload openpgp public key beacuse openpgp keyhandler service returned error"
            )
            return render_template(
                "message.html",
                headline="Upload openpgp public key error",
        ...
        if is_fingerprint_uniq != 0:
            ...
            return render_template(
                "message.html",
                headline="Upload openpgp public key error",
                message="Openpgp public key fingerprint already exist in db",
                current_user=current_user,
            )
        ...
        return render_template(
            "message.html",
            headline="Upload openpgp public key",
            message="Succesfully upload openpgp public key.",
            current_user=current_user,
        )
...
def settings_remove_openpgp_public_key():
    ...
    Request Form Parameters:
        remove_openpgp_public_key (str): Key identifier to remove
        csrf_token (str): CSRF protection token for POST requests

    Error Responses:
        "Failed to remove openpgp public key beacuse this account is disabled": If account is not enabled
        "Failed to remove openpgp public key, validation failed": If key validation fails
        "Failed to remove openpgp public key, openpgp keyhandler service is unavalible": If external service fails
        "The CSRF token is missing": If CSRF token validation fails

    Success Response:
        GET: Renders remove_openpgp_public_key.html template with key removal form
        POST: Renders template with success message confirming key removal
    """
    # Check if cookie secret is set.
    if not "secret" in session:
        current_app.logger.warning("secret is not in session")
    ...
    if request.method == "GET":
        ...
        return render_template(
            "settings_remove_openpgp_public_key.html",
    ...
    if request.method == "POST":
        ...
        if fingerprint == None or fingerprint == "":
            ...
            return render_template(
                "message.html",
                headline="Remove openpgp public key error",
                message="Failed to remove openpgp public key beacuse form is empty",
                current_user=current_user,
            )
        ...
        if is_fingerprint_mine != 1:
            ...
            return render_template(
                "message.html",
                headline="Remove openpgp public key error",
                message="Openpgp public key fingerprint do not exist in database or is not owned by your account",
                current_user=current_user,
            )

        # Remove openpgp public key from database.
        db.session.query(Openpgp_public_key).filter(
            Openpgp_public_key.account_id == current_user.account_id,
            Openpgp_public_key.fingerprint == fingerprint,
        ).delete()
        db.session.commit()
        ...
        if is_fingerprint != 0:
            ...
            return render_template(
                "message.html",
                headline="Remove openpgp public key error",
                message="Openpgp public key fingerprint still exist in database.",
                current_user=current_user,
            )
        ...
        return render_template(
            "message.html",
            headline="Remove OpenPGP Public Key",
            message="Succesfully removed OpenPGP public key.",
            current_user=current_user,
        )
...
def settings_show_emails_with_activated_openpgp():
    ...
    if current_user.account.is_enabled != True:
        ...
        return render_template(
            "message.html",
            headline="Show Emails With Activated Openpgp",
            message="Failed to show emails with activated OpenPGP encryption beacuse this account is disabled. In order to enable the account you need to pay, see payments option in menu.",
            current_user=current_user,
        )

    emails = db.session.query(Email).filter(
        Email.account_id == current_user.account_id, Email.openpgp_public_key_id != None
    )
    ...
    current_app.logger.debug(
        ...
        + " account "
        + current_user.account.account
        + " show emails with active openpgp encryption"
    )
    return render_template(
        "settings_show_emails_with_activated_openpgp.html",
        emails=emails,
        current_user=current_user,
    )
...
def settings_activate_openpgp_encryption():
    """
    Activate OpenPGP encryption for a specific email address.

    This function enables OpenPGP encryption for selected email addresses
    using uploaded public keys. It configures the encryption settings
    through external services and updates the email configuration.

    Returns:
        Response: Flask response with activation result or login redirect

    Request Form Parameters:
        email (str): Email address to enable encryption for
        openpgp_public_key_id (str): Public key ID to use for encryption
        csrf_token (str): CSRF protection token for POST requests

    Error Responses:
        "Failed to activate openpgp encryption beacuse this account is disabled": If account is not enabled
        "Failed to activate openpgp encryption, validation failed": If validation fails
        "Failed to activate openpgp encryption, openpgp keyhandler service is unavalible": If external service fails
        "The CSRF token is missing": If CSRF token validation fails

    Success Response:
        GET: Renders activate_openpgp_encryption.html template with encryption setup form
        POST: Renders template with success message confirming encryption activation
    """
    # Check if cookie secret is set.
    if not "secret" in session:
        current_app.logger.warning("secret is not in session")
    ...
    if current_user.account.is_enabled != True:
        ...
        return render_template(
            "message.html",
            headline="Activate OpenPGP Encryption Error",
            message="Failed to activate OpenPGP encryption beacuse this account is disabled. In order to enable the account you need to pay, see payments option in menu.",
            current_user=current_user,
        )
    ...
    if request.method == "GET":
        ...
        return render_template(
            "settings_activate_openpgp_encryption.html",
            fingerprints=fingerprints,
            emails=emails,
            current_user=current_user,
        )
    if request.method == "POST":
        ...
        if email == None or email == "":
            ...
            return render_template(
                "message.html",
                headline="Activate OpenPGP Encryption Error",
                message="Failed to activate OpenPGP encryption beacuse email form is empty.",
                current_user=current_user,
            )
        ...
        if validators.is_email_allowed(email) == False:
            ...
            return render_template(
                "message.html",
                headline="Activate OpenPGP Encryption Error",
                message="Failed to activate OpenPGP encryption beacuse email validation failed.",
                current_user=current_user,
            )
        ...
        is_email_mine = (
            db.session.query(Email)
            .filter(Email.email == email, Email.account_id == current_user.account_id)
            .count()
        )
        if is_email_mine != 1:
            current_app.logger.warning(
            ...
            return render_template(
                "message.html",
                headline="Activate OpenPGP Encryption Error",
                message="Failed to activate OpenPGP encryption beacuse email can not be found in database",
                current_user=current_user,
            )
        ...
        email_from_db = (
            db.session.query(Email)
            .filter(Email.account_id == current_user.account_id, Email.email == email)
            .first()
        )
        ...
        if (
            ...
            return render_template(
                "message.html",
                headline="Activate OpenPGP Encryption Error",
                message="Failed to activate OpenPGP encryption",
                current_user=current_user,
            )
        ...
        current_app.logger.debug(
            ...
            + " email "
            + email
            + " activated openpgp encryption"
        )
        return render_template(
            "message.html",
            headline="Activate Openpgp Encryption",
            message="Successfully activated OpenPGP encryption",
            current_user=current_user,
        )
...
def settings_deactivate_openpgp_encryption():
    """
    Deactivate OpenPGP encryption for a specific email address.

    This function disables OpenPGP encryption for selected email addresses.
    It removes encryption configuration through external services while
    preserving the public keys for future use.

    Returns:
        Response: Flask response with deactivation result or login redirect

    Request Form Parameters:
        email (str): Email address to disable encryption for
        csrf_token (str): CSRF protection token for POST requests

    Error Responses:
        "Failed to deactivate openpgp encryption beacuse this account is disabled": If account is not enabled
        "Failed to deactivate openpgp encryption, validation failed": If validation fails
        "Failed to deactivate openpgp encryption, openpgp keyhandler service is unavalible": If external service fails
        "The CSRF token is missing": If CSRF token validation fails

    Success Response:
        GET: Renders deactivate_openpgp_encryption.html template with encryption disable form
        POST: Renders template with success message confirming encryption deactivation
    """
    # Check if cookie secret is set.
    if not "secret" in session:
        current_app.logger.warning("secret is not in session")
    ...
    if current_user.account.is_enabled != True:
        ...
        return render_template(
            "message.html",
            headline="Deactivate OpenPGP Encryption Error",
            message="Failed to deactivate OpenPGP encryption beacuse this account is disabled. In order to enable the account you need to pay, see payments option in menu.",
            current_user=current_user,
        )
    ...
    if request.method == "GET":
        ...
        return render_template(
            "settings_deactivate_openpgp_encryption.html",
            emails=emails,
            current_user=current_user,
        )
    if request.method == "POST":
        ...
        if email_from_db.openpgp_public_key_id != None:
            current_app.logger.warning(
                ...
                + " email "
                + email
                + " openpgp encryption is already deactivated"
            )
            return render_template(
                "message.html",
                headline="Deactivate OpenPGP Encryption Error",
                message="Failed to deactivate OpenPGP encryption on this email account",
                current_user=current_user,
            )
        ...
        current_app.logger.debug(
            ...
            + " email "
            + email
            + " deactivated openpgp encryption"
        )
        return render_template(
            "message.html",
            headline="Deactivate Openpgp Encryption",
            message="Successfully deactivated OpenPGP encryption on this email account",
            current_user=current_user,
        )
...
def setings_show_alias():
    ...
    if current_user.account.is_enabled != True:
        ...
        return render_template(
            "message.html",
            headline="Show alias error",
            message="Failed to show alias beacuse this account is disabled. In order to enable the account you need to pay, see payments option in menu.",
            current_user=current_user,
        )
    ...
    return render_template(
        "settings_show_alias.html", aliases=aliases, current_user=current_user
    )
...
def settings_add_alias():
    ...
    This function allows users to create email aliases that forward messages
    from one address to another. It validates the alias configuration and
    creates the necessary forwarding rules in the system.

    Returns:
        Response: Flask response with alias creation result or login redirect

    Request Form Parameters:
        domain (str): Domain name for the alias source address
        src (str): Source address local part for the alias
        dst (str): Destination email address for forwarding
        csrf_token (str): CSRF protection token for POST requests

    Error Responses:
        "ailed to add alias beacuse this account is disabled": If account is not enabled
        "Failed to add alias, validation failed": If alias validation fails
        "The CSRF token is missing": If CSRF token validation fails

    Success Response:
        GET: Renders add_alias.html template with alias creation form
        POST: Renders template with success message confirming alias creation
    """
    # Check if cookie secret is set.
    if not "secret" in session:
        current_app.logger.warning("secret is not in session")
    ...
    if current_user.account.is_enabled != True:
        ...
        return render_template(
            ...
            message="Failed to add alias beacuse this account is disabled. In order to enable the account you need to pay, see payments option in menu.",
            current_user=current_user,
        )
    ...
    if request.method == "POST":
        if not form.validate_on_submit():
            ...
            return render_template(
                "message.html",
                headline="Add alias error",
                message="Failed to add alias, failed csrf validation",
                current_user=current_user,
            )
        else:
            ...
            if validators.is_email_allowed(src_email_from_form) == False:
                ...
                return render_template(
                    "message.html",
                    headline="Add alias error",
                    message="Failed to add alias, source email validation failed.",
                    current_user=current_user,
                )
            ...
            if validators.is_email_allowed(dst_email_from_form) == False:
                ...
                return render_template(
                    "message.html",
                    headline="Add alias error",
                    message="Failed to add alias, destination email validation failed.",
                    current_user=current_user,
                )
            ...
            if validators.is_domain_allowed(validate_src_email_domain[1]) == False:
                ...
                return render_template(
                    "message.html",
                    headline="Add alias error",
                    message="Failed to add alias, source email domain validation failed.",
                    current_user=current_user,
                )
            ...
            is_email_uniq = (
                db.session.query(Email)
                .filter(Email.email == src_email_from_form)
                .count()
            )
            if is_email_uniq != 0:
                current_app.logger.warning(
                    ...
                    + " src email "
                    + src_email_from_form
                    + " exist in email table in db"
                )
                return render_template(
                    "message.html",
                    headline="Add alias error",
                    message="Failed to add alias, source email exist.",
                    current_user=current_user,
                )
            ...
            is_alias_uniq = (
                db.session.query(Alias)
                .filter(Alias.src_email == src_email_from_form)
                .count()
            )
            if is_alias_uniq != 0:
                current_app.logger.warning(
            ...
            is_src_email_domain_global = (
                db.session.query(Global_domain)
                .filter(Global_domain.domain == validate_src_email_domain[1])
                .count()
            )

            if (
                not is_src_email_domain_mine == 1
                and not is_src_email_domain_global == 1
            ):
                current_app.logger.warning(
                    "user "
                    + current_user.user
                    ...
                    + " src email "
                    + src_email_from_form
                    + " domain is not owned by currnet user or not exist in db"
                )
                return render_template(
                    "message.html",
                    headline="Add alias error",
            ...
            dst_email = (
                db.session.query(Email)
                .filter(
                    Email.email == dst_email_from_form,
                    Email.account_id == current_user.account_id,
                )
                .count()
            )
            if dst_email != 1:
                ...
                return render_template(
                    "message.html",
                    headline="Add alias error",
                    message="Failed to add alias, can not find destination email.",
                    current_user=current_user,
                )
            ...
            if is_src_email_domain_mine == 1:
                ...
                new_alias = Alias(
                    account_id=current_user.account_id,
                    src_email=src_email_from_form,
                    src_account_domain_id=src_email_domain.id,
                    dst_email_id=dst_email.id,
                )
                ...
                new_alias = Alias(
                    account_id=current_user.account_id,
                    src_email=src_email_from_form,
                    src_global_domain_id=src_email_global_domain.id,
                    dst_email_id=dst_email.id,
                )
                db.session.add(new_alias)
                db.session.commit()
...
def settings_remove_alias():
    ...
    This function allows users to delete email aliases from their account.
    It removes the forwarding configuration and cleans up the associated
    database records to stop email forwarding.

    Returns:
        Response: Flask response with alias removal result or login redirect

    Request Form Parameters:
        remove_alias (str): Alias identifier to remove
        csrf_token (str): CSRF protection token for POST requests

    Error Responses:
        "Failed to remove alias beacuse this account is disabled": If account is not enabled
        "Failed to remove alias, validation failed": If alias validation fails
        "The CSRF token is missing": If CSRF token validation fails

    Success Response:
        GET: Renders remove_alias.html template with alias removal form
        POST: Renders template with success message confirming alias removal
    """
    # Check if cookie secret is set.
    if not "secret" in session:
        current_app.logger.warning("secret is not in session")
    ...
    if current_user.account.is_enabled != True:
        ...
        return render_template(
            "message.html",
            headline="Remove Alias Error",
            message="Failed to remove alias beacuse this account is disabled. In order to enable the account you need to pay, see payments option in menu.",
            current_user=current_user,
        )
    ...
    if request.method == "GET":
        ...
        return render_template(
            "settings_remove_alias.html", aliases=aliases, current_user=current_user
        )
    if request.method == "POST":
        ...
        if alias_id_from_form.isdigit() != True:
            current_app.logger.warning(
                ...
                + current_user.account.account
                + " alias id "
                + str(alias_id_from_form)
                + " is not a digit"
            )
            return render_template(
                "message.html",
                headline="Remove Alias Error",
                message="Failed to remove alias, validation failed.",
                current_user=current_user,
            )
        ...
        if is_alias_mine != 1:
            current_app.logger.warning(
                ...
                + " alias id "
                + str(alias_id_from_form)
                + " is not owned by current use or is not in db"
            )
            return render_template(
                "message.html",
                headline="Remove Alias Error",
        ...
        db.session.query(Alias).filter(
            Alias.account_id == current_user.account_id, Alias.id == alias_id_from_form
        ).delete()
        db.session.commit()
        ...
        return render_template(
            "message.html",
            headline="Remove Alias",
            message="Successfully removed alias.",
            current_user=current_user,
        )
...
def settings_show_domains():
    """
    Display all custom domains associated with the user's account.

    This function shows a list of custom domains that have been added
    to the user's account for email hosting, including their verification
    status and DNS configuration requirements.

    Returns:
        Response: Flask response with domain list or login redirect

    Request Form Parameters:
        None: This endpoint does not require form parameters

    Error Responses:
        "Failed to show domains beacuse this account is disabled": If account is not enabled

    Success Response:
        Renders show_domains.html template with list of custom domains
    """
    """
    Display all email addresses associated with the user's account.

    This function shows a comprehensive list of all email addresses
    created under the user's account, including their domains and
    configuration status for account management.

    Returns:
        Response: Flask response with email list or login redirect

    Request Form Parameters:
        None: This endpoint does not require form parameters

    Error Responses:
        "Failed to show email beacuse this account is disabled": If account is not enabled

    Success Response:
        Renders show_email.html template with list of account email addresses
    """
    # Check if cookie secret is set.
    if not "secret" in session:
        current_app.logger.warning("secret is not in session")
    ...
    if current_user.account.is_enabled != True:
        ...
        return render_template(
            "message.html",
            headline="Show domains error",
            message="Failed to show domains beacuse this account is disabled. In order to enable the account you need to pay, see payments option in menu.",
            current_user=current_user,
        )
...
def settings_add_domain():
    """
    Add a new custom domain to the user's account.

    This function allows users to add their own domains for email hosting.
    It validates the domain format, checks DNS configuration, and creates
    the necessary database records for domain management.

    Returns:
        Response: Flask response with domain addition result or login redirect

    Request Form Parameters:
        domain (str): Domain name to add to the account
        csrf_token (str): CSRF protection token for POST requests

    Error Responses:
        "Failed to add domain beacuse this account is disabled": If account is not enabled
        "Failed to add domain, the current domain already exist": If domain already exists
        "Failed to add domain, domain validation failed": If domain format is invalid
        "Failed to add domain, form validation failed": If form validation fails
        "The CSRF token is missing": If CSRF token validation fails

    Success Response:
        GET: Renders add_domain.html template with domain addition form
        POST: Renders template with success message confirming domain addition
    """
    # Check if cookie secret is set.
    if not "secret" in session:
        current_app.logger.warning("secret is not in session")
    ...
    if current_user.account.is_enabled != True:
        ...
        return render_template(
            ...
            message="Failed to add domain beacuse this account is disabled.",
            current_user=current_user,
        )
    ...
    if request.method == "POST":
        if not form.validate_on_submit():
            ...
            correct_records = [
                current_app.config["DKIM_CNAME_RECORD1"],
...
def settings_remove_domain():
    ...
    This function allows users to delete custom domains from their account.
    It validates the domain ownership and removes the domain configuration
    and associated database records.

    Returns:
        Response: Flask response with domain removal result or login redirect

    Request Form Parameters:
        remove_domain (str): Domain name to remove from the account
        csrf_token (str): CSRF protection token for POST requests

    Error Responses:
        "Failed to remove domains beacuse this account is disabled": If account is not enabled
        "Failed to remove domain, domain backend validation failed": If domain validation fails
        "Failed to remove domain, domain does not exist or is not owned by your account": If domain not found
        "The CSRF token is missing": If CSRF token validation fails

    Success Response:
        GET: Renders remove_domain.html template with domain removal form
        POST: Renders template with success message confirming domain removal
    """
    # Check if cookie secret is set.
    if not "secret" in session:
        current_app.logger.warning("secret is not in session")
    ...
    if current_user.account.is_enabled != True:
        ...
        return render_template(
            ...
            message="Failed to remove domains beacuse this account is disabled. In order to enable the account you need to pay, see payments option in menu.",
            current_user=current_user,
        )
    ...
    if request.method == "POST":
        ...
        if validators.is_domain_allowed(remove_domain_from_form) == False:
            ...
            return render_template(
                "message.html",
                headline="Remove Domain Error",
                message="Failed to remove domain, domain backend validation failed.",
                current_user=current_user,
            )
        ...
        if is_domain_mine != 1:
            ...
            return render_template(
                "message.html",
                headline="Remove Domain Error",
                message="Failed to remove domain, domain does not exist or is not owned by your account.",
                current_user=current_user,
            )
        ...
        number_off_emails = (
            db.session.query(Email).filter(Email.account_domain_id == domain.id).count()
        )
        number_off_aliases = (
            db.session.query(Alias)
            .filter(Alias.src_account_domain_id == domain.id)
            .count()
        )
        ...
        if number_off_emails != 0 or number_off_aliases != 0:
            current_app.logger.warning(
                ...
                + " domain "
                + remove_domain_from_form
                + " has email or/and alias"
            )
            return render_template(
                "message.html",
                headline="Remove Domain Error",
                message="Failed to remove domain, domain is used in email or alias, remove those first.",
                current_user=current_user,
            )

        # Remove domain account from db.
        db.session.query(Account_domain).filter(
            Account_domain.account_id == current_user.account_id,
            Account_domain.domain == remove_domain_from_form,
        ).delete()
        db.session.commit()
