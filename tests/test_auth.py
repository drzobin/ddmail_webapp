...
def test_login_post_no_data(client, app):
    ...
    response = client.post(
        ...
        data={
            "user": "",
            "password": "",
            "key": (BytesIO(bytes("", "utf-8")), "data.key"),
            "csrf_token": csrf_token_login,
        },
    )
...
def test_register_post_CSRF(client):
    ...
    assert (
        client.post("/register", data={"csrf_token": "test"}).status_code == 400
    )
...
def test_register_post(client):
    ...
    assert (
        client.post("/register", data={"csrf_token": csrf_token}).status_code
        == 200
    )
...
def test_register_login_post(client):
    ...
    assert (
        b"Logged in on account: " + bytes(account, "utf-8")
        in response_login_post.data
    )
    assert (
        b"Logged in as user: " + bytes(username, "utf-8")
        in response_login_post.data
    )
...
def test_register_login_settings_logout(client, app):
    ...
    with app.app_context():
        ...
        db.session.query(User).filter(
            User.user == register_data["username"]
        ).delete()
        db.session.commit()
